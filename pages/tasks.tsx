...
import { Loader2, Archive, ArchiveRestore, User, Calendar, Flag, Briefcase, CheckSquare, Square, Filter, X, LayoutGrid, List, ArrowUpDown, Plus } from 'lucide-react';
...
export default function TasksPage() {
  ...
  const selectAllVisible = useCallback(() => {
    setSelectedTaskIds(() => {
      ...
      for (const t of tasks) {
        ...
        const isVisible = (showArchived ? (isArchived || isRemoving) : (!isArchived || isRemoving)) && matchesScore;
        if (isVisible) next.add(t.id);
      }
      return next;
    });
  }, [tasks, archivedTaskIds, removingTaskIds, showArchived, scoreFilter]);
  ...
  const visibleTaskCount = useMemo(() => {
    ...
    return count;
  }, [tasks, archivedTaskIds, showArchived, removingTaskIds, scoreFilter]);
  ...
  return (
    <div className="flex flex-col h-full bg-background overflow-hidden">
      ...
      <header className="flex-shrink-0 px-6 pt-6 pb-2">
             ...
             <div className="flex items-center bg-muted/30 rounded-lg p-0.5 border border-border/40">
              <Button
                ...
                className={cn(
                  ...
                  !showArchived ? "bg-background text-foreground shadow-sm font-medium" : "text-muted-foreground hover:text-foreground hover:bg-transparent"
                )}
              >
                进行中
              </Button>
        ...
        {/* ── Filter Bar ───────────────────────────────────────────────────── */}
        <div className="flex flex-wrap lg:flex-nowrap items-center gap-2 pb-2 min-w-0">
          ...
          <PeoplePicker
            ...
            className=""
            maxSelections={1}
            triggerClassName="h-8 text-xs rounded-full border-dashed bg-transparent hover:bg-muted/30 px-3 justify-start text-muted-foreground font-normal w-[108px] whitespace-nowrap"
          />

          {/* Status Filter */}
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            <SelectTrigger className="h-8 w-[108px] gap-2 text-xs rounded-full border-dashed bg-transparent hover:bg-muted/30 px-3 whitespace-nowrap">
          ...
          <Select value={projectFilter} onValueChange={setProjectFilter}>
            <SelectTrigger className="h-8 w-[116px] gap-2 text-xs rounded-full border-dashed bg-transparent hover:bg-muted/30 px-3 whitespace-nowrap">
              <div className="flex items-center gap-1.5 text-muted-foreground">
          ...
          {hasActiveFilters && (
            <Button
              ...
              className="h-8 text-xs text-muted-foreground hover:text-foreground px-2 whitespace-nowrap"
            >
          ...
          <div className="flex items-center gap-2 min-w-0 lg:ml-auto whitespace-nowrap">
             <Button
                ...
                className={cn("h-8 text-xs gap-1.5 rounded-full px-3", isBatchMode && "bg-primary/10 text-primary hover:bg-primary/20")}
              >
              ...
              {isBatchMode && (
                <div className="flex items-center gap-1 bg-muted/40 p-1 rounded-full animate-in fade-in slide-in-from-right-2">
                  <span className="text-xs text-muted-foreground mx-2">{selectedTaskIds.size} 选中</span>
                  <Button variant="ghost" size="sm" onClick={selectAllVisible} className="h-6 text-[10px] rounded-full px-2">全选</Button>
                  <Button variant="ghost" size="sm" onClick={clearSelection} className="h-6 text-[10px] rounded-full px-2">清空</Button>
                  ...
                  {showArchived ? (
                    <Button 
                      ...
                      className="h-6 text-[10px] rounded-full px-3 bg-amber-600 hover:bg-amber-700 text-white"
                    >
                      取消归档
                    </Button>
                  ) : (
      ...
      {/* ── Content ──────────────────────────────────────────────────────── */}
      <main className="flex-1 overflow-y-auto min-h-0 px-6 pb-6">
        {loading ? (
            ...
            {visibleTaskCount === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-muted-foreground/40">
                ...
                <p className="text-lg font-medium text-foreground/50">
                  {showArchived ? '没有已归档的任务' : '暂无任务'}
                </p>
                <p className="text-sm mt-1">
                  {showArchived ? '已归档的任务会显示在这里' : '创建一个新任务开始工作'}
                </p>
              </div>
            ) : (
                ...
                {tasks.map((task) => {
                  ...
                  return (
                    <div 
                      ...
                      className={cn(
                        "group relative flex flex-col bg-card hover:bg-accent/5 transition-all duration-200 border rounded-xl overflow-hidden cursor-pointer",
