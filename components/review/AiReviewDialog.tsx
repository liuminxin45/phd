...
export function AiReviewDialog({
  ...
  useEffect(() => {
    ...
    setStep('config');
    setError(null);
  }, [open, changeNumber, revisionId, baseRevisionId]);
  ...
  const sortedCategories: IssueCategory[] = categoryOrder
    .filter((category) => groupedIssues.has(category));
  ...
  const handleOpenChange = (nextOpen: boolean) => {
    ...
    if (!nextOpen) {
      setError(null);
      setLoading(false);
      setStep(result ? 'result' : 'config');
    }
  };

  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-4xl rounded-[28px] border-border/60 p-0">
        ...
        <div className="max-h-[78vh] overflow-y-auto px-6 py-5">
          {loadingRules ? (
            ...
            <div className="space-y-5">
              <div className="rounded-2xl border border-border/50 bg-muted/[0.02] p-4">
                <div className="mb-3 flex items-center gap-2">
                  ...
                  />
                  <Label htmlFor="enable-custom-instructions" className="text-sm font-medium">
                    是否要自定义指令
                  </Label>
                </div>
                {customInstructionEnabled && (
                  <Textarea
                    ...
                    className="min-h-[110px] resize-y bg-background shadow-none"
                  />
                )}
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="rounded-2xl border border-border/50 bg-muted/[0.02] p-4">
                  <div className="mb-3 text-sm font-medium">重点关注的领域</div>
                  <div className="mb-3 flex flex-wrap gap-2">
                    ...
                    )) : <span className="text-xs text-muted-foreground">未设置重点领域</span>}
                  </div>
                  <div className="flex gap-2">
                    <Input
                      ...
                      placeholder="比如：性能、并发、安全"
                      className="shadow-none"
                    />
                    <Button variant="outline" onClick={() => addToken(focusInput, setFocusInput, setFocusAreas)}>
                ...
                <div className="rounded-2xl border border-border/50 bg-muted/[0.02] p-4">
                  <div className="mb-3 text-sm font-medium">忽略的文件</div>
                  <div className="mb-3 flex flex-wrap gap-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    />
                    <Button variant="outline" onClick={() => addToken(ignoreInput, setIgnoreInput, setIgnorePatterns)}>
                      <Plus className="h-4 w-4" />
                    </Button>
                  </div>
                </div>
            ...
            <div className="space-y-5">
              <div className="rounded-2xl border border-border/50 bg-muted/[0.02] p-4">
                <div className="mb-3 text-sm font-medium">二次确认</div>
                <div className="space-y-3 text-sm text-foreground/80">
                  <div>
                    <div className="mb-1 text-xs uppercase tracking-[0.08em] text-muted-foreground">Custom Instructions</div>
                    <p>{rulesOverride.customInstructions || '不使用自定义指令'}</p>
                  </div>
                  <div>
                    <div className="mb-1 text-xs uppercase tracking-[0.08em] text-muted-foreground">Focus Areas</div>
                    <p>{focusAreas.length > 0 ? focusAreas.join('、') : '无额外重点领域'}</p>
                  </div>
                  <div>
                    <div className="mb-1 text-xs uppercase tracking-[0.08em] text-muted-foreground">Ignored Files</div>
                    <p>{ignorePatterns.length > 0 ? ignorePatterns.join(', ') : '无忽略文件规则'}</p>
                  </div>
                </div>
              </div>
              ...
              {loading ? (
                ...
                <>
                  ...
                  {sortedCategories.length > 0 ? (
                      ...
                      {sortedCategories.map((category) => {
                        ...
                        return (
                          <div key={category} className="overflow-hidden rounded-2xl border border-border/50">
                            ...
                            {isExpanded && (
                              <div className="space-y-2 border-t border-border/40 bg-background p-4">
                                {issues.map((issue) => (
                                  <IssueItem
                                    key={issue.id}
                                    issue={issue}
                    ...
                    <div className="rounded-2xl border border-border/50 bg-muted/[0.02] px-4 py-10 text-center text-sm text-muted-foreground">
                      未发现主要问题
                    </div>
                  )}
                </>
        ...
        <DialogFooter className="px-6 py-4">
          ...
          {step === 'confirm' && (
            <>
              <Button variant="ghost" onClick={() => setStep('config')} disabled={loading}>Back</Button>
              <Button onClick={runReview} disabled={loading} className="gap-2">
                {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}
                {loading ? 'Analyzing...' : '开始智能评审'}
              </Button>
            </>
          )}
          {step === 'result' && (
            <>
              <Button variant="ghost" onClick={() => setStep('config')}>Adjust Rules</Button>
        ...
        <div className="min-w-0 flex-1">
          ...
          {issue.file && (
            <div className="mt-1 flex items-center gap-1.5">
              <span className="max-w-[280px] truncate font-mono text-[11px] text-muted-foreground" title={issue.file}>
                {issue.file}
              </span>
