...
export function TaskDetailDialog({ 
  ...
  useEffect(() => {
    ...
    const fetchTaskDetails = async () => {
      ...
      try {
        ...
        if (userPHIDs.size > 0) {
          const usersMap = await httpClient<Record<string, any>>('/api/users/batch', {
            method: 'POST',
            body: { phids: Array.from(userPHIDs) }
          });

          const newUserCache: UserCache = {};
          Object.entries(usersMap).forEach(([phid, userData]) => {
            if (userData) {
              newUserCache[phid] = {
        ...
        if (projectPHIDs.length > 0) {
          const projectData = await httpClient<Record<string, any>>('/api/phid/query', {
            method: 'POST',
            body: { phids: projectPHIDs }
          });

          const newProjectCache: ProjectCache = {};
          Object.entries(projectData).forEach(([phid, data]: [string, any]) => {
            if (data) {
              newProjectCache[phid] = {
                name: data.name || data.fullName || 'Unknown Project',
  ...
  // Background loading function for all subtasks - loads layer by layer and updates UI progressively
  const loadAllSubtasksInBackground = async (rootSubtasks: Subtask[], abortController: AbortController) => {
    try {
      let loadedCount = 0;
      setSubtasksLoadProgress(0);
      
      // Helper to update a specific task's children in the tree
      const updateTaskChildren = (tasks: Subtask[], taskId: number, children: Subtask[], hasChildren: boolean): Subtask[] => {
        return tasks.map(task => {
          if (task.id === taskId) {
            return { ...task, children, hasChildren, isLoadingChildren: false };
          }
          if (task.children && task.children.length > 0) {
            return { ...task, children: updateTaskChildren(task.children, taskId, children, hasChildren) };
          }
          return task;
        });
      };
      
      // Load one level at a time, updating UI after each level
      const loadLevel = async (tasksToLoad: Subtask[], level: number): Promise<Subtask[]> => {
        ...
        // Filter tasks that need loading
        const needsLoading = tasksToLoad.filter(t => t.hasChildren && t.children.length === 0);
        if (needsLoading.length === 0) return [];
        
        // Load all tasks at this level concurrently (batch of 5 to avoid overwhelming)
        const batchSize = 5;
        const allChildren: Subtask[] = [];
        
        for (let i = 0; i < needsLoading.length; i += batchSize) {
          if (abortController.signal.aborted) break;
          
          const batch = needsLoading.slice(i, i + batchSize);
          const batchPromises = batch.map(async (task) => {
            try {
              const subtaskData = await httpClient<any>(`/api/tasks/${task.id}`);
              loadedCount++;
              
              if (subtaskData?.subtasks && subtaskData.subtasks.length > 0) {
                const children: Subtask[] = subtaskData.subtasks
                ...
                return { taskId: task.id, children, hasChildren: true };
              } else {
                return { taskId: task.id, children: [], hasChildren: false };
              }
            } catch (error) {
          ...
          if (!abortController.signal.aborted) {
            setSubtasks(prev => {
              ...
              batchResults.forEach(result => {
                if (result) {
                  updated = updateTaskChildren(updated, result.taskId, result.children, result.hasChildren);
                  allChildren.push(...result.children);
                }
              });
              return updated;
            });
            setSubtasksLoadProgress(loadedCount);
          }
        }
